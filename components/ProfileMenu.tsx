  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1 rounded-full p-0.5 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-40 mt-2 w-60 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
            ...
            {role === "student" && (
              <Link
                href="/student/account?tab=billing"
                onClick={() => setOpen(false)}
                className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50"
              >
                💳 Tuition &amp; payments
